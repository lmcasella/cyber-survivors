...
export class FastWalkState extends State {
    updateEnemy(enemy, player, ticker) {
        // Fast enemy - quick, erratic movement
        if (enemy.isPlayerInAttackRange()) {
            enemy.stateMachine.setState(new FastAttackState());
            return;
        }
...
export class FastAttackState extends State {
    enter(enemy) {
    ...
    updateEnemy(enemy, player, ticker) {
        if (!enemy.isPlayerInAttackRange()) {
            enemy.stateMachine.setState(new FastWalkState());
            return;
        }
